...
/// 🔥 保留了你本地的高度定制，作为后续的默认值
const double kTopNavigationPaneHeight = 72;
      ...
      builder: (context) {
        return PaneItem(
          icon: const Padding(
            ...
            child: Icon(FluentIcons.more, size: 14),
          ),
...
///       openWidth: MediaQuery.widthOf(context) / 5,
///       openMinWidth: 250,
///       openMaxWidth: 320,
///     ),
///   ),
...
class NavigationPaneSize with Diagnosticable {
  ...
  const NavigationPaneSize({
    this.topHeight = kTopNavigationPaneHeight,
...
class _TopNavigationPaneState extends State<_TopNavigationPane> {
  ...
  void _onPressed(PaneItem item) {
    ...
    // lyy 核心修复：强制触发你在 HomePage 里写好的 onTap 路由逻辑！
    item.onTap?.call();
  ...
  Widget build(BuildContext context) {
    ...
    final height = widget.pane.size?.topHeight ?? kTopNavigationPaneHeight;
    return SizedBox(
            ...
            Expanded(
              child: DynamicOverflow(
                ...
                children: _localItemHold.map((index) {
                  ...
                  return SizedBox(
                    height: kCompactNavigationPaneWidth, //lyy
                  ...
                  children: widget.item.items.map((item) {
                    return NavigationPaneTheme.merge(
                      data: const NavigationPaneThemeData(
                        labelPadding: EdgeInsetsDirectional.only(start: 24),
                      ),
                      child: Builder(
                        builder: (context) {
                          return item
